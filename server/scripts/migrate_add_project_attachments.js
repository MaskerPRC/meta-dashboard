...
function migrateDatabase() {
  return new Promise((resolve, reject) => {
    console.log('🔄 开始为projects表添加attachments字段迁移...');
    ...
    const db = new sqlite3.Database(dbPath, (err) => {
      ...
      db.all("PRAGMA table_info(projects)", (err, columns) => {
        if (err) {
          console.error('❌ 获取projects表结构失败:', err);
          reject(err);
          return;
        }
        ...
        if (hasAttachments) {
          console.log('ℹ️ projects表的attachments字段已存在，跳过迁移');
          db.close();
          resolve();
          return;
        }
        ...
        db.run(sql, (err) => {
          if (err) {
            console.error('❌ 为projects表添加attachments字段失败:', err);
            reject(err);
            return;
          }
          
          console.log('✅ projects表attachments字段添加成功');
          ...
          db.all("PRAGMA table_info(projects)", (err, newColumns) => {
            ...
            db.close((closeErr) => {
              if (closeErr) {
                ...
                console.log('✅ projects表attachments字段迁移完成');
                resolve();
              }
            });
          });
...
async function main() {
  try {
    ...
    console.log('🎉 项目attachments字段迁移完成！');
